    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            function ($attribute, $value, $fail) {
                if (!DB::table('df_regkain_polos')->where('kode_kain', $value)->exists() &&
                    !DB::table('df_regkain_printing')->where('kode_kain', $value)->exists()) {
        ...
        if ($regkain != null) {
            ...
            $penerimaanKain = new GJpenerimaan_kain([
                ...
                'kg' => $kg,
                'kainable_id' => 1, // ID dari model DFregkain_polos atau DFregkain_printing
                'kainable_type' => 'App\Models\DFregkain_polos' // Nama namespace model yang digunakan (DFregkain_polos atau DFregkain_printing)
            ]);
            ...
            return redirect()
                ->route('GJpenerimaankain.create')
                ->with('success', 'Penerimaan kain berhasil ditambahkan');
        } elseif ($regkain == null) {
            ...
            $penerimaanKain = new GJpenerimaan_kain([
                ...
                'kode_kain' => $request->kode_kain,
                'kg' => $kg,
                'kainable_id' => 2, // ID dari model DFregkain_polos atau DFregkain_printing
                'kainable_type' => 'App\Models\DFregkain_printing' // Nama namespace model yang digunakan (DFregkain_polos atau DFregkain_printing)
            ]);
